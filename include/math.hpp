...
namespace faze
{
  namespace math
  {
    ...
    struct Vector<2, vType>
    {
      ...
      constexpr Vector() {}
      constexpr Vector(vType v) : x(v), y(v) {}
      constexpr Vector(vType x, vType y) : x(x), y(y) {}
      constexpr Vector(const Vector& o) : x(o.x), y(o.y) {}

      constexpr vType& operator()(unsigned x)
      {
        return data[x];
      }

      constexpr const vType& operator()(unsigned x) const
      {
        return data[x];
      }
    };
    ...
    struct Vector<3, vType>
    {
      ...
      constexpr Vector() {}
      constexpr Vector(vType v) : x(v), y(v), z(v) {}
      constexpr Vector(vType x, vType y, vType z) : x(x), y(y), z(z) {}
      constexpr Vector(const Vector& o) : x(o.x), y(o.y), z(o.z) {}
    ...
    struct Vector<4, vType>
    {
      ...
      constexpr Vector() {}
      constexpr Vector(vType v) : x(v), y(v), z(v), w(v) {}
      constexpr Vector(vType x, vType y, vType z, vType w) : x(x), y(y), z(z), w(w) {}
      constexpr Vector(Vector<3, vType> o, vType v) : x(o.x), y(o.y), z(o.z), w(v) {}
      constexpr Vector(const Vector& o) : x(o.x), y(o.y), z(o.z), w(o.w) {}
    ...
    template<int ELEMENT_COUNT, typename vType>
    constexpr vType length(Vector<ELEMENT_COUNT, vType> a)
    {
      vType val{};
    ...
    constexpr VEC normalize(Vector<ELEMENT_COUNT, vType> a)
    {
      VEC n{};
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr vType dot(Vector<ELEMENT_COUNT, vType> a, Vector<ELEMENT_COUNT, vType> b)
    {
      vType v{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    // scalars
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC mul(Vector<ELEMENT_COUNT, vType> a, vType b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC mul(vType b, Vector<ELEMENT_COUNT, vType> a)
    {
      return mul(a, b);
    }

    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC div(Vector<ELEMENT_COUNT, vType> a, vType b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC add(Vector<ELEMENT_COUNT, vType> a, vType b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC add(vType b, Vector<ELEMENT_COUNT, vType> a)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC sub(Vector<ELEMENT_COUNT, vType> a, vType b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC sub(vType b, Vector<ELEMENT_COUNT, vType> a)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    // vectors
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC mul(Vector<ELEMENT_COUNT, vType> a, Vector<ELEMENT_COUNT, vType> b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC div(Vector<ELEMENT_COUNT, vType> a, Vector<ELEMENT_COUNT, vType> b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC add(Vector<ELEMENT_COUNT, vType> a, Vector<ELEMENT_COUNT, vType> b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<int ELEMENT_COUNT, typename vType, typename VEC = Vector<ELEMENT_COUNT, vType>>
    constexpr VEC sub(Vector<ELEMENT_COUNT, vType> a, Vector<ELEMENT_COUNT, vType> b)
    {
      VEC n{};
      for (int i = 0; i < ELEMENT_COUNT; ++i)
      {
    ...
    template<typename vType>
    constexpr Vector<3, vType> crossProduct(Vector<3, vType> a, Vector<3, vType> b)
    {
      Vector<3, vType> r;
      r.x = a.y * b.z - a.z * b.y;
      r.y = a.z * b.x - a.x * b.z;
    ...
    template<typename vType>
    constexpr Vector<4, vType> crossProduct(Vector<4, vType> a, Vector<4, vType> b)
    {
      Vector<4, vType> r{};
      r.x = a.y * b.z - a.z * b.y;
      r.y = a.z * b.x - a.x * b.z;
    ...
    struct Matrix
    {
      ...
      constexpr vType& operator()(unsigned x, unsigned y)
      {
        return data[y * columnCount + x];
      }
      constexpr const vType& operator()(unsigned x, unsigned y) const
      {
        return data[y * columnCount + x];
      }
      ...
      constexpr Vector<columnCount, vType> row(unsigned y)
      {
    ...
    struct Quaternion
    {
      ...
      constexpr float& operator()(unsigned x)
      {
        return data[x];
      }

      constexpr const float& operator()(unsigned x) const
      {
        return data[x];
      }
    };
    ...
    constexpr Quaternion rotateAxis(float x, float y, float z, float amount)
    {
      Quaternion l{};
    ...
    constexpr Quaternion rotateAxis(float3 directionUnitVector, float amount)
    {
      Quaternion l{};
      l.w = std::cos(amount / 2.f);
    ...
    constexpr Quaternion mul(Quaternion q1, Quaternion q2)
    {
      Quaternion r{};
      r.w = q1.w * q2.w;
      for (int i = 1; i < 4; i++)
    ...
    constexpr Quaternion rotationQuaternionSlow(float yaw, float pitch, float roll)
    {
    ...
    constexpr Quaternion rotationQuaternionEuler(float yaw, float pitch, float roll)
    {
      Quaternion q{};
    ...
    constexpr Matrix<cols, rows, vType> transpose(Matrix<rows, cols, vType> value)
    {
    ...
    constexpr Matrix<rows, cols2, vType> mul(Matrix<rows, cols, vType> a, Matrix<rows2, cols2, vType> b)
    {
      // a = <2, 3>, b = <3, 2>
      // rows = 2
      // cols2 = 2
    ...
    template <typename vType, int rows, int cols>
    Matrix<1, cols, vType> mul(Vector<cols, vType> v, Matrix<rows, cols, vType> a)
    {
    ...
    template <typename vType, int rows, int cols>
    Matrix<rows, 1, vType> mul(Matrix<rows, cols, vType> a, Vector<rows, vType> v)
    {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> mul(Matrix<rows, cols, vType> a, vType b)
    {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> mul(vType b, Matrix<rows, cols, vType> a)
    {
      return mul(a, b);
    }

    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> div(Matrix<rows, cols, vType> a, vType b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> add(Matrix<rows, cols, vType> a, Matrix<rows, cols, vType> b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> add(Matrix<rows, cols, vType> a, vType b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> add(vType b, Matrix<rows, cols, vType> a)
    {
      return add(a,b);
    }

    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> sub(Matrix<rows, cols, vType> a, Matrix<rows, cols, vType> b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> sub(Matrix<rows, cols, vType> a, vType b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> sub(vType b, Matrix<rows, cols, vType> a)
    {
      return sub(a,b);
    }

    template <typename vType, int rows, int cols, int rows2, int cols2>
    constexpr Matrix<1, rows*cols + rows2*cols2, vType> concatenateToSingleDimension(Matrix<rows, cols, vType> a, Matrix<rows2, cols2, vType> b)
    {
    ...
    template <typename vType, int rows, int cols>
    constexpr Matrix<rows, cols, vType> mulElementWise(Matrix<rows, cols, vType> a, Matrix<rows, cols, vType> b)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int i = 0; i < rows*cols; ++i)
      {
    ...
    constexpr Matrix<rows, cols, vType> transform(Matrix<rows, cols, vType> value, Func&& f)
    {
      Matrix<rows, cols, vType> outResult{};
      for (int y = 0; y < rows; ++y)
      {
    ...
    float4x4 translation(float3 vec)
    {
      float4x4 result = float4x4::identity();
      result(0, 3) = vec.x; result(1,3) = vec.y; result(2,3) = vec.z;
      return result;
    }
    float4x4 translation(float4 vec)
    {
      float4x4 result = float4x4::identity();
    ...
    float4x4 perspectivelh(float fov, float aspect, float NearZ, float FarZ)
    {
      if (fov <= 0 || aspect == 0)
      {
        return float4x4{};
    ...
    float4x4 perspectiverh(float fov, float aspect, float NearZ, float FarZ)
    {
    ...
    float4x4 lookAt(float4 cameraPos, float4 cameraTarget, float4 upVec = float4(0.f, 1.f, 0.f, 0.f))
    {
     ...
     */
    float4x4 rotationMatrixRH(float yaw, float pitch, float roll)
    {
    ...
    float4x4 rotationMatrixRH(Quaternion q)
    {
      auto r = float4x4::identity();
    ...
    float4x4 rotationMatrixLH(Quaternion q)
    {
      auto r = float4x4::identity();
      float xx = std::pow(q.x, 2.f);
      float yy = std::pow(q.y, 2.f);
